...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		kashdMessage, err := client.Post(&protowire.KashdMessage{Payload: &protowire.KashdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := kashdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	kashdMessage := &protowire.KashdMessage{}
	err := protojson.Unmarshal([]byte(response), kashdMessage)
	if err != nil {
	...
	return marshalOptions.Format(kashdMessage)
}
