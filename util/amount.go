...
// monetary amount.
const (
	AmountMegaKSH  AmountUnit = 6
	AmountKiloKSH  AmountUnit = 3
	AmountKSH      AmountUnit = 0
	AmountMilliKSH AmountUnit = -3
	AmountMicroKSH AmountUnit = -6
	AmountSompi    AmountUnit = -8
)

// String returns the unit as a string. For recognized units, the SI
// prefix is used, or "Sompi" for the base unit. For all unrecognized
// units, "1eN KSH" is returned, where N is the AmountUnit.
func (u AmountUnit) String() string {
	...
	case AmountSompi:
		return "Sompi"
	default:
		return "1e" + strconv.FormatInt(int64(u), 10) + " KSH"
	}
}
...
//
// NewAmount is for specifically for converting KSH to Sompi.
// For creating a new Amount with an int64 value which denotes a quantity of Sompi,
// do a simple type conversion from type int64 to Amount.
// TODO: Refactor NewAmount. When amounts are more than 1e9 KSH, the precision
...
func NewAmount(f float64) (Amount, error) {
	...
	return round(f * constants.SompiPerKash), nil
}
...
// ToKSH is the equivalent of calling ToUnit with AmountKSH.
func (a Amount) ToKSH() float64 {
	return a.ToUnit(AmountKSH)
}
...
// String is the equivalent of calling Format with AmountKSH.
func (a Amount) String() string {
	return a.Format(AmountKSH)
}
