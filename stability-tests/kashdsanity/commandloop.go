...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("kashdsanity-kashd-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("kashd", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "KASHD-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "KASHD-STDERR")
