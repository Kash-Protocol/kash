...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("kashd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("kashd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("KASHD-SYNCER",
		"kashd",
	...
	syncedCmd, err := common.StartCmd("KASHD-SYNCED",
		"kashd",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
