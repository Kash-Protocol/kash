...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of kashd instances
	kashd1, kashd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := kashd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In kashd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, kashd2)
	}

	// Connect the two kashds. This should trigger sync
	// between the two nodes
	connect(t, kashd1, kashd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := kashd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
